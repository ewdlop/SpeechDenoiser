...
using static TorchSharp.torch.fft;
using NumSharp;
...
class Program
{
    static void Main()
    {
        ...
        var conv_cache = np.zeros(new Shape(2, 1, 16, 16, 33), dtype: np.float32);
        var tra_cache = np.zeros(new Shape(2, 3, 1, 1, 16), dtype: np.float32);
        var inter_cache = np.zeros(new Shape(2, 1, 33, 16), dtype: np.float32);
    ...
    static NDArray ToNDArray(DenseTensor<float> tensor)
    {
